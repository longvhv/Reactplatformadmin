/**
 * SaasProductTypeDetailPage Component
 * Detail page for SaaS Product Type
 */
...
export default function SaasProductTypeDetailPage() {
  ...
  const fetchProductType = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to fetch product type');
  ...
  const handleToggleActive = async () => {
    ...
    try {
      if (productType.is_active) {
        ...
        showToast.success('Thành công', 'Đã vô hiệu hóa');
      } else {
        await saasProductTypesApi.activate(productType._id);
        showToast.success('Thành công', 'Đã kích hoạt');
      }
      fetchProductType();
    } catch (err: any) {
  ...
  const handleEdit = () => {
    navigate(`/commerce/saas-product-types/edit/${id}`);
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <Button onClick={() => navigate('/commerce/saas-product-types')} className="mt-4">
            Quay lại
          </Button>
        </div>
      </div>
