...
export default function NotificationTemplatesPage() {
  ...
  const loadTemplates = async () => {
    try {
      ...
      showToast.error(t('notificationTemplates.loadError', { error: error.message }));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (data: Partial<NotificationTemplate>) => {
    try {
      ...
      if (editingTemplate?._id) {
        ...
        showToast.success(t('notificationTemplates.updateSuccess'));
      } else {
        await notificationTemplateApi.create(data as Omit<NotificationTemplate, '_id'>);
        showToast.success(t('notificationTemplates.createSuccess'));
      }
      setShowForm(false);
      setEditingTemplate(undefined);
      loadTemplates();
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await notificationTemplateApi.delete(selectedTemplateId);
      showToast.success(t('notificationTemplates.deleteSuccess'));
      loadTemplates();
    } catch (error: any) {
      showToast.error(error.message);
    } finally {
  ...
  const handleToggleStatus = async (template: NotificationTemplate) => {
    try {
      ...
      showToast.success(t('notificationTemplates.statusUpdated'));
      loadTemplates();
    } catch (error: any) {
      showToast.error(error.message);
    }
  ...
  const handleDuplicate = async (template: NotificationTemplate) => {
    try {
      ...
      showToast.success(t('notificationTemplates.duplicateSuccess'));
      loadTemplates();
    } catch (error: any) {
      showToast.error(error.message);
    }
    ...
    <>
      <div className="min-h-screen bg-gray-50">
        ...
        <ConfirmDialog
          ...
          description={t('notificationTemplates.deleteMessage')}
          confirmLabel="Xóa"
          cancelLabel="Hủy"
          variant="destructive"
        />
