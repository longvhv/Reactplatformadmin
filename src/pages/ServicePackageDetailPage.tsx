 ...
 * Hiển thị chi tiết gói dịch vụ với sidebar layout
...
export default function ServicePackageDetailPage() {
  ...
  const loadServicePackage = async () => {
    try {
      ...
      showToast.error('Lỗi', 'Không thể tải gói dịch vụ: ' + error.message);
      navigate('/commerce/service-packages');
    } finally {
      setLoading(false);
    }
  ...
  const handleToggleActiveConfirm = async () => {
    ...
    try {
      ...
      showToast.error('Lỗi', 'Không thể cập nhật trạng thái: ' + error.message);
    }
  ...
  const handleCloneConfirm = async () => {
    ...
    try {
      const clonedPackage = await cloneServicePackage(servicePackage._id, cloneCode);
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        description={`Bạn có chắc chắn muốn xóa gói dịch vụ "${servicePackage.name}"? Hành động này không thể hoàn tác.`}
        confirmLabel="Xóa"
        cancelLabel="Hủy"
        variant="destructive"
      />
      ...
      <ConfirmDialog
        ...
        description={`Bạn có chắc chắn muốn ${servicePackage.status === 'ACTIVE' ? 'vô hiệu hóa' : 'kích hoạt'} gói dịch vụ này?`}
