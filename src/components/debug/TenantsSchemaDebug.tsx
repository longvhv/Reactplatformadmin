...
export function TenantsSchemaDebug() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Tenants Schema Debugger</h1>
      ...
      <Alert>
        ...
        <AlertDescription>
          Checking <code>tenants</code> for column existence (specifically <code>is_deleted</code> vs <code>deleted_at</code>).
        </AlertDescription>
      </Alert>

      <Card className={`border ${errors['tenants'] ? 'border-red-200 bg-red-50' : 'border-green-200 bg-green-50'}`}>
        <CardHeader>
          <CardTitle className={`flex items-center ${errors['tenants'] ? 'text-red-700' : 'text-green-700'}`}>
            {errors['tenants'] ? <XCircle className="w-5 h-5 mr-2" /> : <CheckCircle2 className="w-5 h-5 mr-2" />}
            Table: tenants
          </CardTitle>
        </CardHeader>
        <CardContent>
          {errors['tenants'] ? (
            <pre className="bg-white p-4 rounded border overflow-auto text-xs text-red-600">
              {JSON.stringify(errors['tenants'], null, 2)}
            </pre>
          ) : (
            <div className="space-y-4">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(schemaInfo['tenants']) ? (
                    schemaInfo['tenants'].map((col: string) => (
                    ...
                    <span className="text-sm text-gray-600">{schemaInfo['tenants']}</span>
                  )}
                </div>
              </div>
              ...
              {results['tenants'] && results['tenants'].length > 0 && (
                <div>
                  ...
                  <pre className="bg-white p-4 rounded border overflow-auto text-xs">
                    {JSON.stringify(results['tenants'][0], null, 2)}
                  </pre>
                </div>
              )}
            </div>
