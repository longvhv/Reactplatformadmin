/**
 * PackageRevenueTab - Revenue analytics for package
 */
...
interface PackageRevenueTabProps {
  packageId: string;
}

interface PackageRevenueByMonth {
  month: string;
  revenue: number;
...
export function PackageRevenueTab({ packageId }: PackageRevenueTabProps) {
  const [revenues, setRevenues] = useState<PackageRevenueByMonth[]>([]);
  ...
  const fetchRevenue = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/v1/packages/${packageId}/revenue?months=${months}`);
  ...
  const totalChurned = revenues.reduce((sum, r) => sum + r.churned, 0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-gray-600">Rời bỏ</p>
              <p className="text-2xl font-bold text-gray-900">{totalChurned}</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={revenues}>
            ...
            <Bar dataKey="new_subscribers" fill={CHART_COLORS.success} name="Khách hàng mới" />
          </BarChart>
        </ResponsiveContainer>
      </Card>
            ...
            <tbody>
              {revenues.map((rev) => (
                <tr key={rev.month} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 text-sm text-right text-orange-600 font-semibold">
                    -{rev.churned}
                  </td>
                </tr>
              ))}
