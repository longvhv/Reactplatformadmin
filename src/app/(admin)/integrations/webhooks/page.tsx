            ...
            <div className="space-y-2">
              {webhooks.map((webhook) => (
                <div 
                  ...
                  onClick={() => router.push(`/integrations/webhooks/${webhook._id}`)}
                >
                  <div>
                    <p className="font-medium text-gray-900 dark:text-white hover:text-indigo-600 transition-colors">
                      {webhook.url}
                    </p>
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`text-xs px-2 py-0.5 rounded ${webhook.is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300'}`}>
                        {webhook.is_active ? 'Active' : 'Inactive'}
                      </span>
                      <span className="text-sm text-gray-500 dark:text-gray-400">
                        {webhook.event_type || 'All Events'}
                      </span>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button 
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/integrations/webhooks/${webhook._id}`);
                      }}
                    >
                      View
                    </Button>
                    <Button 
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/integrations/webhooks/edit/${webhook._id}`);
                      }}
                    >
                      Edit
                    </Button>
                    <Button 
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleDelete(webhook._id, webhook.url);
                      }}
                    >
                      Delete
                    </Button>
