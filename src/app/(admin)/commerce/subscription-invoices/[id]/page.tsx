...
import { 
  ...
  Receipt, Package, User, ShoppingCart, Calculator
} from 'lucide-react';
...
export default function SubscriptionInvoiceDetailPage() {
  ...
  const loadInvoice = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        showToast.error(t('invoices.errors.notFound') || 'Invoice not found');
        router.push('/commerce/subscription-invoices');
      }
    } catch (error) {
      console.error('Error loading invoice:', error);
      showToast.error(t('invoices.errors.loadFailed') || 'Failed to load invoice');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      showToast.success(t('invoices.deleteSuccess') || 'Invoice deleted successfully');
      router.push('/commerce/subscription-invoices');
    } catch (error) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      showToast.success(t('invoices.sendSuccess') || 'Invoice sent successfully');
      loadInvoice();
    } catch (error) {
      console.error('Error sending invoice:', error);
      showToast.error(t('invoices.errors.sendFailed') || 'Failed to send invoice');
    }
  };
  ...
  const formatCurrency = (amount: number | undefined | null, currencyCode: string) => {
    // FIX: Handle undefined/null/NaN to avoid "NaN" display
  ...
  const calculateFinalAmount = () => {
    if (!invoice) return 0;
    // Prefer total_amount if available, otherwise calculate using amount/subtotal and adjustments
    if (invoice.total_amount !== undefined && invoice.total_amount !== null) {
      return invoice.total_amount;
    }
    const base = invoice.subtotal || invoice.amount || 0;
    return subscriptionInvoiceApi.calculateTotal(base, invoice.price_adjustments || []);
  };

  // Helper to get consistent data
  const getBillingInfo = () => invoice?.billing_info || invoice?.customer_snapshot;
  const getItems = () => invoice?.items_snapshot || invoice?.line_items || [];
    ...
    return (
      <div className="p-6">
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">{t('invoices.notFound') || 'Invoice not found'}</h3>
          <div className="mt-6">
            <Button onClick={() => router.push('/commerce/subscription-invoices')}>
              {t('common.back') || 'Back'}
            </Button>
          </div>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button variant="ghost" onClick={() => router.push('/commerce/subscription-invoices')}>
              <ArrowLeft className="h-4 w-4" />
            </Button>
            <div>
              <h1 className="text-3xl font-bold text-gray-900">{invoice.invoice_number}</h1>
              <p className="text-gray-500 mt-1">
                {t('invoices.createdOn') || 'Created on'} {formatDate(invoice.created_at)}
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => router.push(`/commerce/subscription-invoices/edit/${invoice._id}`)}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle>{t('invoices.invoiceDetails') || 'Invoice Details'}</CardTitle>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-gray-600">{t('invoices.invoiceNumber') || 'Invoice Number'}</p>
                    <p className="font-medium font-mono">{invoice.invoice_number}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Subscription ID</p>
                    <p className="font-medium font-mono text-xs">{invoice.subscription_id || 'N/A'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-sm text-gray-600">{t('invoices.currency') || 'Currency'}</p>
                    <p className="font-medium">{invoice.currency_code}</p>
                  </div>
                  {invoice.order_id && (
                    <div>
                      <p className="text-sm text-gray-600">Order ID</p>
                      <p className="font-medium font-mono text-xs">{invoice.order_id}</p>
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            {billingInfo && (
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-4">
                    {(billingInfo.name || billingInfo.customer_name) && (
                      <div>
                        <p className="text-sm text-gray-600">Customer Name</p>
                        <p className="font-medium">{billingInfo.name || billingInfo.customer_name}</p>
                      </div>
                    )}
                    {billingInfo.tax_id && (
                      <div>
                        <p className="text-sm text-gray-600">Tax ID</p>
                        <p className="font-medium font-mono">{billingInfo.tax_id}</p>
                      </div>
                    )}
                    {(billingInfo.email || billingInfo.customer_email) && (
                      <div>
                        <p className="text-sm text-gray-600">Email</p>
                        <p className="font-medium text-blue-600">{billingInfo.email || billingInfo.customer_email}</p>
                      </div>
                    )}
                    {(billingInfo.phone || billingInfo.customer_phone) && (
                      <div>
                        <p className="text-sm text-gray-600">Phone</p>
                        <p className="font-medium">{billingInfo.phone || billingInfo.customer_phone}</p>
                      </div>
                    )}
                    {billingInfo.address && (
                        ...
                        {items.map((item, index) => (
                          <tr key={index} className="border-b last:border-0">
                            ...
                            <td className="text-right py-3 font-semibold">
                              {formatCurrency(item.total || (item.price * item.qty), invoice.currency_code)}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-xl font-bold pt-3 border-t">
                    ...
                    <span className="text-indigo-600">
                      {formatCurrency(calculateFinalAmount(), invoice.currency_code)}
                    </span>
                  </div>
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between pt-2 border-t font-bold">
                      ...
                      <span className="text-indigo-600">
                        {formatCurrency(
                          invoice.tax_breakdown.reduce((sum, tax) => sum + (tax.amount || 0), 0),
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t('common.auditTrail') || 'Audit Trail'}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 text-sm">
                <div>
                  <p className="text-gray-600">{t('common.createdAt') || 'Created At'}</p>
                  <p className="font-medium">{formatDate(invoice.created_at)}</p>
                </div>
                <div>
                  <p className="text-gray-600">{t('common.updatedAt') || 'Updated At'}</p>
                  <p className="font-medium">{formatDate(invoice.updated_at)}</p>
                </div>
                <div>
                  <p className="text-gray-600">{t('common.version') || 'Version'}</p>
