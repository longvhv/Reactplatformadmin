...
function EditServicePackagePage() {
  ...
  const loadData = async () => { try { setDataLoading(true); const result = await servicePackagesApi.getById(id); setData(result); } catch (error: any) { showToast.error('Error', 'Failed'); } finally { setDataLoading(false); } };
  const handleSubmit = async (formData: any) => { setLoading(true); try { await servicePackagesApi.update(id, formData); showToast.success('Success', 'Updated'); router.push('/platform/service-packages'); } catch (error: any) { showToast.error('Error', error.message || 'Failed'); } finally { setLoading(false); } };
  ...
  return <FormPageLayout mode="edit" title="Edit Service Package" description="Update service package" icon={Package} backPath="/platform/service-packages" backLabel="Back"><ServicePackageForm initialData={data} onSubmit={handleSubmit} loading={loading} onCancel={() => router.push('/platform/service-packages')} /></FormPageLayout>;
}
